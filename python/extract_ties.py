...
- Simple ties (start → end)
- Tie chains (start → both → both → ... → end)
            ...
            if tie_to:
                ...
                if primary_href:
                    ...
                    if secondary_href:
                        ...
                        print(f"🔗 Found tie: {primary_clean} → {secondary_href}")
                    else:
                        print(f"⚠️  Could not find href for target ID: {target_id}")
                else:
                    print(f"⚠️  Could not find href for tie start element with role: {tie_role}")
            ...
            if primary_href:
                ...
                if secondary_href:
                    ...
                    if tie_pair not in ties:
                        ...
                        print(f"🔗 Found tie (fallback): {primary_href} → {secondary_clean}")
                else:
                    print(f"⚠️  Could not find href for tie end element")
            else:
                print(f"⚠️  Could not find href for source ID: {source_id}")
...
def clean_href_path(href):
    ...
    Example:
        "textedit:///work/file.ly:10:5:6" → "file.ly:10:5:6"
    """
...
def main():
    ...
    try:
        ...
        if new_ties:
            print()
            print("📈 Tie Analysis Summary:")
            print(f"   🔗 Total tie relationships: {len(new_ties)}")
            ...
            for file_name, file_ties in tie_files.items():
                print(f"   📄 {file_name}: {len(file_ties)} tie(s)")
